...
static CommandInfo command_database[] = {
    ...
    // Expression commands
    {"infix2postfix", "Convert infix to postfix", "infix2postfix <expr>", {"infix2postfix \"5 + 3 * 2\"", "", ""}},
    {"infix2prefix", "Convert infix to prefix", "infix2prefix <expr>", {"infix2prefix \"5 + 3 * 2\"", "", ""}},
    {"postfix2infix", "Convert postfix to infix", "postfix2infix <expr>", {"postfix2infix \"5 3 + 2 *\"", "", ""}},
    {"prefix2infix", "Convert prefix to infix", "prefix2infix <expr>", {"prefix2infix \"+ 5 * 3 2\"", "", ""}},
    {"evaluate", "Evaluate postfix expression", "evaluate <expr>", {"evaluate \"5 3 + 2 *\"", "", ""}},
    {"visualeval", "Visualize postfix evaluation", "visualeval <expr>", {"visualeval \"5 + 3 * 2\"", "", ""}},
    {"visualfix", "Visualize infix to postfix", "visualfix <expr>", {"visualfix \"a + b * c\"", "", ""}},
    {"visualpost", "Visualize postfix to infix", "visualpost <expr>", {"visualpost \"a b + c *\"", "", ""}},
    {"exprtype", "Detect expression type", "exprtype <expr>", {"exprtype \"5 + 3\"", "", ""}},
    // Encoding/decoding commands
    {"base64", "Base64 encode/decode", "base64 <encode|decode> <text>", {"base64 encode hello", "base64 decode aGVsbG8=", ""}},
    {"morse", "Morse code encode/decode", "morse <encode|decode> <text>", {"morse encode hello", "", ""}},
    {"rot13", "ROT13 cipher", "rot13 <text>", {"rot13 hello", "", ""}},
    {"binary", "Convert to binary", "binary <number>", {"binary 42", "", ""}},
    {"hex", "Convert to hex", "hex <number>", {"hex 255", "", ""}},
    {"octal", "Convert to octal", "octal <number>", {"octal 64", "", ""}},
    // Compression
    {"compress", "Compress file", "compress <file>", {"compress data.txt", "", ""}},
    {"decompress", "Decompress file", "decompress <file>", {"decompress data.txt.rle", "", ""}},
    // Utility commands
    {"uuid", "Generate UUID", "uuid", {"uuid", "", ""}},
    {"random", "Generate random number", "random [max]", {"random", "random 100", ""}},
    {"timer", "Countdown timer", "timer <seconds>", {"timer 10", "", ""}},
    {"stopwatch", "Stopwatch", "stopwatch", {"stopwatch", "", ""}},
    {"colorpalette", "Show color palette", "colorpalette", {"colorpalette", "", ""}},
    {"lorem", "Generate lorem ipsum", "lorem [sentences]", {"lorem", "lorem 5", ""}},
    {"calendar", "Show calendar", "calendar [month] [year]", {"calendar", "calendar 1 2026", ""}},
    {"json", "JSON pretty print", "json <file>", {"json data.json", "", ""}},
    {"categories", "Show command categories", "categories", {"categories", "", ""}},
    {"teach", "Teaching mode", "teach [on|off]", {"teach on", "teach off", ""}},
    {"uname", "System information", "uname [-a]", {"uname", "uname -a", ""}},
};
...
void suggestion_get_commands(const char *prefix, SuggestionList *out) {
    ...
    for (int i = 0; i < num_command_infos && out->count < MAX_SUGGESTIONS; i++) {
        char lower_cmd[64] = {0};
        size_t cmd_len = strlen(command_database[i].name);
        memcpy(lower_cmd, command_database[i].name, cmd_len < 63 ? cmd_len : 63);
        str_to_lower_sug(lower_cmd);
        
        if (strncmp(lower_cmd, lower_prefix, prefix_len) == 0) {
            memset(out->suggestions[out->count], 0, MAX_SUGGESTION_LEN);
            size_t name_len = strlen(command_database[i].name);
            memcpy(out->suggestions[out->count], command_database[i].name, name_len < MAX_SUGGESTION_LEN - 1 ? name_len : MAX_SUGGESTION_LEN - 1);
            out->count++;
        }
    }
...
void suggestion_get_paths(const char *partial_path, int dirs_only, SuggestionList *out) {
    ...
    while ((entry = readdir(dir)) != NULL && out->count < MAX_SUGGESTIONS) {
        ...
        if (stat(full_path, &st) == 0) {
            ...
            memcpy(out->suggestions[out->count], suggestion, sug_len < MAX_SUGGESTION_LEN - 1 ? sug_len : MAX_SUGGESTION_LEN - 1);
            out->count++;
        }
    }
