...
3. [Detailed Data Structure Analysis](#3-detailed-data-structure-analysis)
   ...
   - [3.8 Expression Processing Stacks](#38-expression-processing-stacks)
...
| Expression Stacks | Expression conversion/evaluation | `expression.c` |
...
### 3.8 Expression Processing Stacks

**Location:** `backend/src/expression.c`

**Purpose:** Convert and evaluate mathematical expressions between infix, postfix, and prefix notations.
...
// String stack for multi-character operands/operators
typedef struct {
    char *data[MAX_LEN];
    int top;
} StringStack;
...
#### Visual Representation - Expression Conversion
...
| Operation | Time Complexity | Space Complexity | Description |
|-----------|----------------|------------------|-------------|
| `char_push()` | O(1) | O(1) | Push character to stack |
| `char_pop()` | O(1) | O(1) | Pop character from stack |
| `string_push()` | O(m) | O(m) | Push string (length m) |
| `string_pop()` | O(1) | O(1) | Pop string pointer |
| `num_push()` | O(1) | O(1) | Push number |
| `num_pop()` | O(1) | O(1) | Pop number |
| `infix2postfix()` | O(n) | O(n) | Convert n-token expression |
| `infix2prefix()` | O(n) | O(n) | Convert with string reversal |
| `evaluate_postfix()` | O(n) | O(n) | Evaluate n-token expression |

#### Implementation Example - Postfix Evaluation
...
double evaluate_postfix(const char *expr) {
    ...
    for (int i = 0; i < token_count; i++) {
        if (is_operator(tokens[i])) {
            double b = num_pop(&stack);
            double a = num_pop(&stack);
            double result;
            ...
            num_push(&stack, result);
        } else {
            // Operand - convert to number and push
            num_push(&stack, atof(tokens[i]));
        }
    }
    
    return num_pop(&stack);  // Final result
}
```
...
// Used in infix2postfix conversion
while (stack.top >= 0 && 
       precedence(stack.data[stack.top]) >= precedence(op) &&
       !is_right_associative(op)) {
    // Pop higher/equal precedence operators
}
```

#### Evaluation Example Walkthrough
...
1. `infix2postfix` - Convert mathematical expressions
2. `infix2prefix` - Convert to prefix notation
3. `postfix2infix` - Convert back to infix
4. `prefix2infix` - Convert prefix to infix
5. `evaluate` - Evaluate infix expressions
6. `visualeval` - Step-by-step evaluation display
7. `visualfix` - Show infix conversion steps
8. `visualpost` - Show postfix conversion steps
9. `exprtype` - Identify expression notation type
...
Step 1: Push 5 → [5]
Step 2: Push 3 → [5, 3]
Step 3: Pop 3,5 → Add → Push 8 → [8]
Step 4: Push 2 → [8, 2]
Step 5: Pop 2,8 → Multiply → Push 16 → [16]
Result: 16
```
...
```c
// String stack requires explicit cleanup
void string_free(StringStack *stack) {
    while (stack->top >= 0) {
        free(stack->data[stack->top--]);
    }
}

// Tokenization allocates strings that must be freed
...
| Notation | Evaluation | Conversion | Human Readable |
|----------|-----------|------------|----------------|
| **Infix** | O(n) complex | N/A | ✅ Yes |
| **Postfix** | O(n) simple | O(n) | ❌ No |
| **Prefix** | O(n) simple | O(n) | ❌ No |
...
| Stack (Undo) | O(1) | O(n) | O(1) | O(n) |
| Stack (Expression) | O(1) | N/A | O(1) | O(n) |
...
| Infix→Postfix | O(n) | O(n) | Expression conversion |
| Postfix Evaluation | O(n) | O(n) | Calculate result |
